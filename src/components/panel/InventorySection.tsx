...
=======
        <div className="flex gap-1 rounded-2xl bg-slate-100 p-1">
          {(['movimientos', 'proveedores', 'alertas'] as SubTab[]).map(t => (
            <button key={t} onClick={() => setActiveSubTab(t)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all capitalize ${activeSubTab === t ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-white/50'}`}>{t}</button>
>>>>>>> origin/develop
          ))}
        </div>
      </header>
