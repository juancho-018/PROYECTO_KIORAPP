              ...
              {(Array.isArray(filteredProducts) ? filteredProducts : []).map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="h-32 w-full bg-slate-50 rounded-xl border border-slate-100 overflow-hidden mb-3 flex items-center justify-center shrink-0">
                      {p.imagen_prod ? (
                        <img src={p.imagen_prod} alt={p.nom_prod} className="h-full w-full object-contain transition-transform duration-500 group-hover:scale-110" />
                      ) : (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {(!orderForm.items || orderForm.items.length === 0) ? (
                ...
                (orderForm.items || []).map((item) => {
                  const p = filteredProducts.find(x => x.cod_prod === item.cod_prod);
                  const maxStock = p?.stock_actual ?? (item as any).maxStock ?? 999;
                  const isMax = item.cantidad >= maxStock;

                  return (
                    <div key={item.cod_prod} className="flex gap-4 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm animate-in slide-in-from-right-2 duration-300 transition-all hover:border-slate-300">
                      {/* Product Image */}
                      <div className="h-16 w-16 bg-slate-50 rounded-xl border border-slate-100 overflow-hidden shrink-0 flex items-center justify-center">
                        {item.url_imagen ? (
                          <img src={item.url_imagen} alt={item.nom_prod} className="h-full w-full object-contain" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <h5 className="text-sm font-bold text-slate-800 line-clamp-2 leading-tight pr-4">{item.nom_prod}</h5>
                        <div className="flex items-center justify-between mt-auto">
                          <div className="flex items-center gap-0.5 border border-slate-200 rounded-lg p-0.5 bg-slate-50 w-fit">
                            ...
                            <button 
                              onClick={() => updateQuantity(item.cod_prod, 1, maxStock)} 
                              disabled={isMax}
                              className={`w-7 h-7 flex items-center justify-center bg-white rounded-md shadow-sm border border-slate-100 font-black transition-colors focus:outline-none ${isMax ? 'opacity-30 cursor-not-allowed' : 'hover:text-[#ec131e]'}`}
