    ...
    <>
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full z-40 w-56 bg-surface border-r border-outline-variant/50 pt-16 shadow-sm">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
          {groups.map((group) => (
            <div key={group.label}>
              <h3 className="px-3 text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-wider mb-2">
                {group.label}
              </h3>
              <ul className="space-y-1">
                {group.items.map((item) => {
                  ...
                  const tourNavIds: Record<string, string> = {
                    ...
                    estructura: 'tour-nav-estructura',
                  };
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-surface border-t border-outline-variant/50 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]"
        ...
        <div className="flex items-center justify-between px-2 py-2.5">
          {['dashboard', 'ventas', 'productos', 'inventario'].map((coreId) => {
            const item = allItems.find(i => i.id === coreId);
            if (!item) return null;
            const active = activeId === item.id;
            return (
              <button
                ...
                type="button"
                onClick={() => onItemClick(item.id)}
                className={`flex-1 flex flex-col items-center justify-center p-1.5 transition-all duration-200 active:scale-95 ${
          ...
          <button
            ...
            className="flex-1 flex flex-col items-center justify-center p-1.5 transition-all duration-200 active:scale-95 text-on-surface-variant/60"
          >
            <div className="flex items-center justify-center hover:scale-105 transition-transform duration-200">
              <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>menu</span>
            </div>
            <span className="text-[10px] mt-1 font-medium truncate w-full text-center">Más</span>
          </button>
        </div>
      </nav>
      ...
      {mobileMoreOpen && (
        <div className="md:hidden fixed inset-0 z-[300] flex flex-col justify-end">
          <div className="absolute inset-0 bg-inverse-surface/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setMobileMoreOpen(false)} />
          <div className="relative w-full bg-surface rounded-t-3xl shadow-2xl p-6 animate-in slide-in-from-bottom duration-300 max-h-[85vh] overflow-y-auto">
            <div className="mx-auto w-12 h-1.5 bg-outline-variant/50 rounded-full mb-6" />
            
            <div className="space-y-6">
              {groups.map((group) => (
                <div key={group.label}>
                  <h3 className="text-xs font-bold text-primary mb-3 uppercase tracking-wider">{group.label}</h3>
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center gap-2 p-2 rounded-xl transition-all ${
                            active ? 'bg-primary-container/30 text-primary' : 'text-on-surface-variant hover:bg-surface-container-high'
                          }`}
                        >
                          <div className={`p-2 rounded-full ${active ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface'}`}>
                            {item.icon}
                          </div>
            ...
            <button
              ...
              className="mt-8 w-full py-3.5 rounded-xl bg-surface-container-high text-on-surface font-medium hover:bg-surface-container-highest transition-colors"
            >
              Cerrar
            </button>
          </div>
