...
import { inventoryService, productService, orderService, alertService } from '@/config/setup';
...
export function DashboardSection({ onSwitchTab }: DashboardSectionProps) {
  ...
  useEffect(() => {
    ...
    // Live refresh support (when stock moves in other parts of the app)
    const handleRefresh = () => loadDashboardData();
    window.addEventListener('kiora-refresh-alerts', handleRefresh);
    
    // Auto-poll every minute for cross-client changes
    const poll = setInterval(loadDashboardData, 60000);
  ...
  const todayOrders = orders.filter(o => o.fecha_vent && new Date(o.fecha_vent).toDateString() === new Date().toDateString());
  
  const stats = [
    { label: 'Ventas de Hoy', value: 'ND', icon: 'money', color: 'text-emerald-500', bg: 'bg-emerald-50', iconSvg: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { label: 'Pedidos Kiosco', value: todayOrders.length > 0 ? todayOrders.length.toString() : orders.length.toString(), icon: 'clipboard', color: 'text-blue-500', bg: 'bg-blue-50', iconSvg: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-600 ring-1 ring-emerald-200">
          ...
          {isLoading ? 'Conectando...' : 'En línea (BD Conectada)'}
        </div>
      </header>

      {/* NEW: Alertas de Sistema */}
      <SystemAlerts />
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-slate-800">Kiora Insights (En Vivo)</h3>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="flex items-start gap-4 rounded-2xl bg-amber-50/50 p-6 ring-1 ring-amber-200/50">
            ...
            <div>
              <h4 className="font-bold text-slate-900">Actividad de Pedidos</h4>
              <p className="mt-1 text-sm leading-relaxed text-slate-600 font-medium">
                Se han procesado <span className="font-bold text-orange-600">{orders.length} pedidos</span> almacenados desde el backend.
              </p>
            </div>
          </div>

          {/* Stock Insight */}
          <div className={`flex items-start gap-4 rounded-2xl p-6 ring-1 transition-all duration-500 ${criticalStock.length > 0 ? 'bg-red-50 ring-red-200 animate-in pulse-red' : 'bg-red-50/50 ring-red-200/50'}`}>
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${criticalStock.length > 0 ? 'bg-red-600 text-white animate-bounce' : 'bg-red-100 text-red-600'}`}>
               <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {criticalStock.length > 0 ? (
                <>
                  <p className="mt-1 text-sm text-slate-600 font-medium">
                    ¡Atención! Hay <span className="text-red-600 font-bold">{criticalStock.length} productos</span> agotándose rápidamente.
                  </p>
                  <button 
                    onClick={() => onSwitchTab('inventario')}
                    className="mt-4 w-full rounded-xl bg-red-600 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-red-200 transition-all hover:bg-red-700 active:scale-95"
                  >
                    Reponer Stock Ahora
                  </button>
                </>
      ...
      <section className="rounded-3xl border border-slate-200/60 bg-white p-6 shadow-sm">
        <div className="mb-8 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-800">Tráfico de Pedidos Semanal</h3>
        </div>
