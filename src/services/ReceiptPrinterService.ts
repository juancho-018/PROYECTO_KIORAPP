 ...
 *
 * Compatibilidad:
 *  - WebSerial:    Chrome 89+, Edge 89+
 *  - WebBluetooth: Chrome 56+ (desktop), Edge 79+
 *  - window.print: todos los navegadores
 */
...
// Most Peripage / Phomemo / Cat Printer / MX10 devices share these
...
function buildEscPosTicket(order: Order, businessName = 'KIORA'): Uint8Array {
  ...
  const buf: number[] = [
    ...
    ...Array.from(encode('COMPROBANTE DE COMPRA\n')),
    ...CMD_ALIGN_LEFT,
    ...divider('-', W),
    // Info
...
// These printers use a simplified protocol: plain UTF-8 text chunked in 20-byte frames.
// Some support basic formatting; all support plain text + line feeds.
function buildBlePackets(order: Order, businessName = 'KIORA'): Uint8Array[] {
  const items  = order.items ?? [];
  const total  = Number(order.montofinal_vent ?? 0);
  const date   = order.fecha_vent
    ? new Date(order.fecha_vent).toLocaleString('es-CO')
  ...
  // Chunk into 20-byte BLE packets (BLE MTU safe size)
...
export class ReceiptPrinterService {
  ...
  generateReceiptHTML(order: Order, businessName = 'KIORA'): string {
    const items  = order.items ?? [];
    const total  = Number(order.montofinal_vent ?? 0);
    const date   = order.fecha_vent
      ? new Date(order.fecha_vent).toLocaleString('es-CO')
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    td.item-name { width: 44%; }
    td.item-qty, td.item-price, td.item-subtotal { text-align: right; white-space: nowrap; }
...
<body>
  ...
  // ── 1. Browser print (any system printer / Save as PDF) ──────────────────────
  printWithBrowser(order: Order, businessName = 'KIORA'): void {
    ...
    if (!win) {
      alert('Permite las ventanas emergentes de este sitio para imprimir.');
      return;
    }
  ...
  // ── 2. WebSerial — USB/Serial ESC/POS thermal printers ───────────────────────
  ...
  async printWithSerial(order: Order, businessName = 'KIORA'): Promise<void> {
  ...
  // ── 3. WebBluetooth BLE — mini thermal printers (Peripage, Phomemo, MX10…) ──
  isBleConnected(): boolean {
    return this.bleChar !== null && this.bleDevice?.gatt?.connected === true;
  }
  ...
  async printWithBluetooth(order: Order, businessName = 'KIORA'): Promise<void> {
    if (!this.bleChar) throw new Error('Impresora Bluetooth no conectada');

    const packets = buildBlePackets(order, businessName);

    // Write packets with a small delay to avoid BLE congestion
  ...
  getBleDeviceName(): string | null {
    return this.bleDevice?.name ?? null;
  }
}
